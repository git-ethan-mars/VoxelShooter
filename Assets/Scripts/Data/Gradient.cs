...
namespace Data
{
	...
	public class Gradient
	{
		...
		public Color CalculateGradient(float value)
		{
			...
			(float h1, float s1, float l1) = RGB2HSL(start);
			(float h2, float s2, float l2) = RGB2HSL(end);
			float newHue = (h2 - h1) * percent + h1;
			float newSaturation = (s2 - s1) * percent + s1;
			float newLightness = (l2 - l1) * percent + l1;
