...
namespace GamePlay
{
	public abstract class Explosive : Entity
	{
		...
		protected void Explode(ExplosionData explosionData)
		{
			...
			{
				DestroyVoxels(mapDestruction, explosionData);
			}

			// Damage is taken by the entities' damage visitor components, which the container does not hold itself.
			foreach (Entity entity in EntityContainer.GetEntitiesByType<Entity>().ToList())
			{
				if (entity != null && entity.TryGetComponent(out IDamageVisitor visitor))
				{
