...

{
    public class VoxelHealthSystem
    {
        ...
        public VoxelHealthSystem(int width, int height, int depth, IStaticDataService staticData)
        {
            _height = height;
            _depth = depth;
            var healthBalance = staticData.GetVoxelHealthBalance();    
            ...
            _healthByBlock = new int[width * height * depth];
        ...
        public List<Voxel> DamageVoxels(List<Voxel> voxels, IVoxelDamageCalculator damageCalculator)
        {
            ...
            for (var i = 0; i < voxels.Count; i++)
            {
                var previousHealth = GetVoxelHealth(voxels[i]);
                if (previousHealth == 0)
                {
                    continue;
                }
                
                var currentHealth = Math.Max(previousHealth - damageCalculator.CalculateDamage(voxels[i]), 0);
                SetVoxelHealth(voxels[i], currentHealth);
                var color = CalculateVoxelColor(voxels[i].Data, currentHealth, previousHealth);
