...
{
    public class PaletteColorCreator
    {
        ...
        public readonly Dictionary<int, Color> ColorById;

        public PaletteColorCreator()
        {
            ColorById = new Dictionary<int, Color>();
            ...
            for (var i = 0; i < colorBoarders.Length; i++)
            {
                ...
                {
                    ColorById[j + i * colorBoarders.Length] =
                        CreateGradient(colorBoarders[i].StartColor, colorBoarders[i].EndColor, (float) j / 8);
                }
            }
        }

        private Color CreateGradient(Color startValue, Color endValue, float percent)
        {
