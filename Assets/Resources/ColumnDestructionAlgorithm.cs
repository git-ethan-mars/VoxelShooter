...
public class ColumnDestructionAlgorithm
{
    ...
    public void Add(Vector3Int blockPosition)
    {
        var singleVoxelRun = new Run(blockPosition.x, blockPosition.z, blockPosition.y, 1, true);
        if (!TryMergeRuns(singleVoxelRun, _columns[blockPosition.x * _mapProvider.MapData.Depth + blockPosition.z]))
        {
            AddRun(singleVoxelRun);
        }
    }

    public List<Vector3Int> Remove(List<Vector3Int> removingPositions)
    {
        ...
        {
            for (var height = runsToDelete[i].Begin; height < runsToDelete[i].Begin + runsToDelete[i].Length; height++)
            {
